...
function show_tip() {
    wxalert('网速不好，请分享到微信群，可免费加速观看', '好')
}
$(function() {
...
$(function () {
    ...
    $.ajax({
        type : "GET",
        url : "http://outku9.zhongquankeji.net/wxshare?url=" + location.href.split('#')[0] + '&_=' + Date.now(),
        dataType : "json",
        data:{},
        success : function(result){
            window.data = result.data.share;
            adurl =  result.data.share.ad_url;
            $("#js_view_source").attr("href",adurl);//
...
function zp() {
    location.href=adurl//"http://mp.weixin.qq.comm.aicuntao.net/qq/news.html";
}
